...
4. Verify Korean punctuation, particles, spacing, dependent-noun/auxiliary-verb usage, Japanese-translationese naturalness, voice/register, terminology, protected multiword proper nouns, renderer-width line breaks, and glyph coverage against the actual game consumer. Apply comma/period spacing according to each renderer's half-width/full-width space behavior, and apply the shared ellipsis rule across dialogue, battle text, subtitles/STR, and other user-visible consumers: attach the ellipsis to the preceding word, then use one space before continued speech when the sentence continues. For every game, first establish per-consumer display budgets (dialogue, choices, item acquisition, item help/descriptions, system popups, battle messages) including maximum rendered width, lines per page, pages, margins, and dynamic/icon width where known. Re-run those layout budgets after every translation or terminology change; treat item acquisition/help as mandatory high-risk checks. If a line cannot fit without risking meaning, proper nouns, honorific/register, quantities, controls, or polarity such as can/cannot, report the exact overage and alternatives to the user instead of silently truncating or changing meaning. Treat naturalness regexes as candidate generators, not blind replacements.
...
7. Verify changed binary structures at their real consumer boundary: spans, records, physical windows, pointers, sizes, decoded/raw size, raw/packed descriptors, allocator-visible sizes, alignment, compression descriptors, allocation spans, archives, graphics structures, and physical layout as applicable. If relocation/reference completeness is not proved, preserve known-good fixed sub-offsets, slot boundaries, alignment, and overall file/container size rather than inferring safety from the absence of an obvious pointer table.
...
11. When parallel translation is used, keep canonical integration single-writer: workers produce isolated drafts, and one integration session applies only reviewed/new groups then reruns global QA. Treat batch merge as atomic, do not checkpoint failed states, and rerun cumulative/global QA after accepted batches so later terminology, codebook, font, or builder changes cannot silently regress prior PASS ranges.
